...
- (void)viewDidLoad
{
    ...
    [self.redoButton addTarget:self action:@selector(performRedo:) forControlEvents:UIControlEventTouchUpInside];
...
- (void) saveNoteWithContent:(NSString *)newContent {
    ...
    if (newContent != currentText) {
        ...
        [self.detailItem.undoManager registerUndoWithTarget:self
    ...
    self.undoButton.enabled = [self.detailItem.undoManager canUndo];
    self.redoButton.enabled = [self.detailItem.undoManager canRedo];
